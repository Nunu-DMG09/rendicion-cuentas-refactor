...
export default function EjesTable({
	ejes,
	onToggleEstado,
	isLoading,
}: EjesTableProps) {
	...
	return (
		...
		>
			<div className="bg-linear-to-r from-primary-dark to-primary p-5">
						...
						{isLoading ? (
							...
							paginatedEjes.map((eje, index) => (
								...
								>
									<td className="px-6 py-4">
										<div className="flex items-center gap-3">
											<div className="w-9 h-9 bg-primary-dark/10 rounded-lg flex items-center justify-center">
									...
									<td className="px-6 py-4 text-center text-gray-600 text-sm">
										{formatDate(eje.created_at)}
									</td>
									<td className="px-6 py-4 text-center">
										<motion.button
											onClick={() =>
												onToggleEstado(eje.id)
											}
											className={`
