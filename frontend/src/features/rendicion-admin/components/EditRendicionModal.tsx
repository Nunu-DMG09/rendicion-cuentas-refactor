	...
	const handleAddBanners = useCallback((files: FileList) => {
		...
		Array.from(files).forEach((file) => {
			...
			if (file.size > MAX_FILE_SIZE_MB * 1024 * 1024) {
				newErrors.push(
					`${file.name}: Excede el tamaño máximo de ${MAX_FILE_SIZE_MB}MB`
				);
				return;
			}
	...
	return (
		<AnimatePresence>
			{isOpen && rendicion && (
				...
				>
					<motion.div
						className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto scrollbar-thin scrollbar-track-transparent scrollbar-thumb-gray-300 my-8"
						variants={modalVariants}
						onClick={(e) => e.stopPropagation()}
					>
						<div className="bg-linear-to-r from-primary-dark to-primary p-6 sticky top-0 z-10">
						...
						{updateSuccess && (
							...
							>
								...
								<span className="text-green-700 font-medium">
									¡Rendición actualizada exitosamente!
								</span>
							</motion.div>
						)}
						{updateError && (
						...
						<form onSubmit={handleSubmit} className="p-6 space-y-6">
							...
							<div>
								...
								<p className="text-sm text-gray-500 mt-2">
									Si no selecciona nuevos banners, se
