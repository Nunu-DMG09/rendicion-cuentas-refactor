...
const getStatusBadge = (status: UpcomingRendicion["status"]) => {
	switch (status) {
		case "programada":
			return (
				<span className="px-2 py-1 text-xs font-medium bg-blue-100 text-primary-dark rounded-full">
					Programada
				</span>
			);
		case "en_curso":
		...
		>
			<div className="flex items-center justify-between mb-6">
				<h3 className="text-lg font-semibold text-gray-900">
					Próximas Rendiciones
				</h3>
				<button
					className="cursor-pointer text-sm text-primary-dark hover:underline font-medium"
			...
			<div className="space-y-4">
				{rendiciones.map((rendicion, index) => (
					<motion.div
						key={rendicion.id}
						className="p-4 rounded-xl border border-gray-100 hover:border-primary-dark/20 hover:bg-blue-50/30 transition-all cursor-pointer"
					...
					>
						<div className="flex items-start justify-between mb-3">
							...
							{getStatusBadge(rendicion.status.toLocaleLowerCase())}
						</div>
						...
						<div className="flex items-center space-x-4">
							<div className="flex items-center text-sm text-gray-600">
								...
								<span>{rendicion.registrados} registrados</span>
							</div>
							<div className="flex items-center text-sm text-gray-600">
								<FaQuestionCircle className="h-3 w-3 mr-1 text-yellow-500" />
								<span>
									{rendicion.preguntasCount} preguntas
								</span>
							</div>
						</div>
					</motion.div>
